...
interface SimpleProductGroupCardProps {
  productGroup: ProductGroup;
  className?: string;
}

export function SimpleProductGroupCard({ productGroup, className = '' }: SimpleProductGroupCardProps) {
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    // Create a simple product for cart
          ...
          {productGroup.variants && productGroup.variants.length > 1 && (
            <div className="absolute bottom-3 left-3">
              <div className="flex space-x-1">
                {productGroup.availableMetalTypes && productGroup.availableMetalTypes.slice(0, 3).map((metalType) => (
                  <div
                    key={metalType}
                    className="w-4 h-4 rounded-full border-2 border-white shadow-md bg-gray-400"
                    title={metalType}
                  />
                ))}
                {productGroup.availableMetalTypes && productGroup.availableMetalTypes.length > 3 && (
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center space-x-2">
              <span className="text-xs px-2 py-1 bg-gray-100 rounded-full text-gray-600">
                {productGroup.baseVariant?.metalType || 'Unknown Metal'}
              </span>
              <span className="text-xs px-2 py-1 bg-gray-100 rounded-full text-gray-600">
                {productGroup.baseVariant?.metalKarat || 'Unknown Karat'}
              </span>
            </div>
