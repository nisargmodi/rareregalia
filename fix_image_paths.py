...
def find_local_images_for_sku(sku, vendor_data_path):
    ...
    base_sku = sku.split('-')[0]  # Get base SKU like "19012" from "19012-W-0.5"
    
    # Try multiple SKU patterns
    possible_skus = [
        base_sku,           # e.g., "19012"
        f"SR-{base_sku}",   # e.g., "SR-19012" 
        f"R-{base_sku}",    # e.g., "R-19012"
    ]
    
    # Also handle cases where base_sku already has prefix
    if base_sku.startswith('SR-'):
        clean_sku = base_sku[3:]  # Remove SR- prefix
        possible_skus.extend([clean_sku, base_sku])
...
def fix_product_image_paths():
    ...
    website_path = Path('.')  # Current directory is now the ecommerce-website directory
...
if __name__ == "__main__":
    ...
    if success:
        print("\n[SUCCESS] Image paths fixed successfully!")
